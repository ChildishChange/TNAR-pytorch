...
def train(model,train_loader,optimizer,device,epoch):
    ...
    print('====> Epoch: {} Average bce loss: {:.4f} Average kld loss: {:.4f}'.format(
          epoch, bce_loss / len(train_loader.dataset), kld_loss / len(train_loader.dataset)))
...
if __name__ == "__main__":
    ...
    for e in range(1, args.epoch + 1):
        ...
        if e%10 == 0: 
            torch.save(model.state_dict(), args.logdir+'/model-'+str(e)+'.pkl')
        with torch.no_grad():
            samples = model.decode(sample).cpu()
            save_image(samples.view(64, 3, 32, 32),args.resultdir+'/sample_' + str(e) + '.png')
